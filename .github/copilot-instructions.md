...
- The web server document root should point at the project root for shared-hosting deployments.
- Root contains pages and assets, `includes/` contains shared layout/config helpers, `auth/` contains login and guard logic, `api/` contains form endpoints, `database/` contains schema and PDO setup, and `emails/` contains HTML email templates.
...
- For local development, run `php -S localhost:8000` from the project root.
